...
def MITO_GENERATED_RECON_FUNCTION_Commercial_European_Real_Estate():
    # Imported df1 using get_european_estate_data
    df1 = get_european_estate_data(sector='Commercial')
    ...
    # Filtered Stategy
    Prologis_v1 = Prologis_v1[Prologis_v1['Stategy'].apply(lambda val: all(val != s for s in ['Office', 'Mixed Use']))]
    ...
    df1_tmp = df1.drop(['Peak space utilization', 'Country', 'Space utilization', 'Portfolio', 'Square Meters'], axis=1)
    df4_tmp = temp_df.drop(['Peak space utilization', 'Country', 'Space utilization', 'Portfolio', 'Square Meters'], axis=1)
    ...
    # Added column 'Net Effect Rental Check'
    df5.insert(10, 'Net Effect Rental Check', CHECK_NUMBER_DIFFERENCE(df5['Net Effective Rent_df4'],df5['Net Effective Rent_df4'], 10))
